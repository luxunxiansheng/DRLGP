...
class Board:
    ...
    B2, for example, stands for the location (column B , row 2)
    ...
    def get_player_id(self, point):
        return self._grid.get(point)

    def place(self, player, point):
        assert self.is_on_grid(point)
        assert self.get_player_id(point) is None
        self._grid[point] = player.id
    ...
    def is_valid_point(self, point):
        return (self.get_player_id(point) is None)
    ...
    def print_board(self):
        ...
        for row in range(1, self._board_size+1):
            ...
            for col in range(1, self._board_size+1):
                player_id = self.get_player_id(Point(row, col))
                pieces.append(player_id) if player_id is not None else pieces.append('')
