    ...
    def select_move(self, game):
        ...
        for _ in tqdm(range(self._num_rounds), desc='Rollout Loop'):
            ...
            if not game.is_final_state(node.game_state):
                ...
                board_matrix = self._encoder.encode(
                    game_state_memory.game_states, node.game_state.player_in_action, node.game_state.previous_move)
                estimated_priors, estimated_state_value = self._predict(
                    board_matrix)
                ...
                for point in free_points:
                    ...
                    node.add_child(
                        game, point, estimated_priors[0][idx].item())
            else:
        ...
        for idx in range(self._encoder.num_points()):
            ...
            if point in self.mcts_tree.working_node.children:
                ...
                num_visits = self.mcts_tree.working_node.get_child(
                    point).num_visits
