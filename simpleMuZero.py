...
class Game(object):
    ...
    def make_image(self, state_index: int):
        ...
        state = [black_ary, white_ary] if o.player_turn() == Player.black else [
            white_ary, black_ary]
...
def muzero(config: MuZeroConfig):
    ...
    for _ in range(config.num_actors):
        t = threading.Thread(target=launch_job, args=(
    ...
    while True:
        ...
        for i in range(config.training_steps):
            if i % config.checkpoint_interval == 0 and i > 0:
                ...
                print('network_vs_random = ', sorted(
                    vs_random_once.items()), end='\n')
                vs_older = latest_vs_older(
                    storage.latest_network(), storage.old_network())
                print('lastnet_vs_older = ', sorted(
                    vs_older.items()), end='\n')
    ...
    for image, actions, targets in batch:
        ...
        for prediction, target in zip(predictions, targets):
            if(len(target[2]) > 0):
                ...
                p_loss += torch.sum(-torch.Tensor(numpy.array(target_policy)
                                                  ).to(device) * torch.log(policy_logits))
                v_loss += torch.sum(
                    (torch.Tensor([target_value]).to(device) - value) ** 2)
...
def softmax_sample(distribution, temperature: float):
    if temperature == 0:
        temperature = 1
    distribution = numpy.array(distribution)[:,0]
